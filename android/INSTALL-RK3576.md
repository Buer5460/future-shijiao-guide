...
安装文件：`outputs/FutureShijiao-RK3576-Android14-Compatible-1.4.1.apk`
...
新包版本号为 `1.4.1 (6)`，与已确认可安装的 1.2.0 使用相同包名、测试证书、SDK、Manifest 和权限结构，正常情况下可直接覆盖安装。完整离线资料经过兼容压缩，APK 约 370KB。
...
```bash
adb install -r FutureShijiao-RK3576-Android14-Compatible-1.4.1.apk
```
...
adb install FutureShijiao-RK3576-Android14-Compatible-1.4.1.apk
```
...
同时在设备“设置 → 应用”中确认 Android System WebView 或 Chrome 已启用。图片中的设备没有获取到 IP 地址，不影响 1.4.1 兼容离线版运行。
