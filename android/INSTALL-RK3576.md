...
安装文件：`outputs/FutureShijiao-RK3576-Offline-1.4.0.apk`
...
新包版本号为 `1.4.0 (5)`，与旧包使用同一测试证书，正常情况下可直接覆盖安装。
...
```bash
adb install -r FutureShijiao-RK3576-Offline-1.4.0.apk
```
...
adb install FutureShijiao-RK3576-Offline-1.4.0.apk
```
...
同时在设备“设置 → 应用”中确认 Android System WebView 或 Chrome 已启用。图片中的设备没有获取到 IP 地址，不影响 1.4.0 离线版运行。
